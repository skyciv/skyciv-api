...
// This PHP example uses cURL to make the request to the SkyCiv API
// Please make sure your PHP installation uses cURL, otherwise use an alternative ethod to make the HTTPS POST request
...
// Get credentials from JSON file if it exists
if (file_exists('./../credentials.json')) $credentials = json_decode(file_get_contents('./../credentials.json'));
...
$model_data = json_decode($model_data); // convert to PHP associative array
...
// cURL POST request to endpoint
$ch = curl_init('https://api.skyciv.com:8085/v3');
